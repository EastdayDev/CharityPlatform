...
angular.module('userModule').factory('_user', ['_http', '_cookie', 'appKey',
  function(_http, _cookie, appKey) {
    ...
    service.checkLogin = function(callback) {
      var requireLogin = !_cookie.get(appKey);
      if (callback) callback(requireLogin);
    }
    ...
    service.login = function(userName, password, callback) {
      ...
      _http.ajaxGet(apiController, 'Usp_User_Login', user, function(data) {
        if (data && data.length > 0) {
          service.user = data[0];
          service.userId = service.user.Id;
          _cookie.put(appKey, service.user, {
            expireMinutes: 180
          });
    ...
    service.Usp_User_List = function(filterValue, pageIndex, pageSize,
      callback) {
      var param = {
